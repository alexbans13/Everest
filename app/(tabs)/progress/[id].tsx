...
export default function ProgressScreen() {
  ...
  const loadJourney = useCallback(async (syncProgress = false) => {
    try {
      setLoading(true);
      
      // Sync progress from health data for this specific journey first
      if (syncProgress && id) {
        setSyncing(true);
        try {
          console.log('Syncing progress for user journey:', id);
          await syncJourneyProgressFromHealthData(id);
        } catch (error) {
  ...
  useEffect(() => {
    // Load journey and sync progress on initial load
    loadJourney(true);
  }, [loadJourney]);
  ...
  useFocusEffect(
    useCallback(() => {
      // Sync progress and reload when screen comes into focus
      loadJourney(true);
    }, [loadJourney])
  );
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.content}>
        ...
        {userJourney.journey?.image_url && (
          <Image
            source={{ uri: userJourney.journey.image_url }}
